...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for data_loader in data_loaders:
            for data, label in tqdm(data_loader):
                # for my_data, label in data_loader:
    ...
    print('Average sizes: [{:2.2f}+/-{:2.2f}] x [{:2.2f}+/-{:2.2f}] = [{:2.2f}+/-{:2.2f} Mpix]'.format(np.mean(Hs),
                                                                                                       np.std(Hs),
                                                                                                       np.mean(Ws),
                                                                                                       np.std(Ws),
                                                                                                       np.mean(
    ...
    if (not opt.size_only):
        ...
        print('AP: {:2.2f}, Acc: {:2.2f}, Acc (real): {:2.2f}, Acc (fake): {:2.2f}'.format(ap * 100., acc * 100.,
                                                                                           r_acc * 100., f_acc * 100.))
